            ...
            <View style={{ flex: 1, margin: 10, backgroundColor: "#fff" }}>
                <ScrollView
                    ...
                    showsVerticalScrollIndicator={false}>
                    {
                        list?.map((item, index) => {
                            return (
                                <TouchableOpacity onPress={() => handleNavigate(item)} style={{ width: "100%", flexDirection: "row", backgroundColor: item?.readStatus ? "#f0f3f5" : "#fff", alignItems: "flex-start", padding: 6, borderRadius: 10, justifyContent: "space-between", borderBottomColor: "#f8f8f8", borderBottomWidth: 1 }} key={index}>
                                    ...
                                    <Text style={{ width: "60%", fontSize: 12, color: "#000", fontFamily: "OpenSans-SemiBold" }}>{item?.title?.length > 50 ? item?.title?.substring(0, 50) + "..." : item?.title}</Text>
                    ...
                    {list?.length > 12 && <ActivityIndicator size={"small"} />}
                </ScrollView>
            </View>
